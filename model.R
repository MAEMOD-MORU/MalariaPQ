...
Malaria_model_with_Array_2 <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    dS <- mui * N - muo * S - (prob_lam*lam * S) - ((1-prob_lam)*lam * S) + alpha * Rs + alpha * Rr
    
    # dIs <- -muo * Is + lam_s * S * prob_s * propIs - gamma_i * Is - tau_ntsd0 * Is[1] - (1-gamma_i-muo) * Is[2:3] # Array operation for Is
    dIs <- -muo * Is + (prob_lam*lam * S) * prob_s * propIs - Is * (gamma_is)
    ...
    dAs <- -muo * As + (prob_lam*lam * S) * (1 - prob_s) - tau_as * As - gamma_as * As
    ...
    dIr <- -muo * Ir + ((1-prob_lam)*lam * S) * prob_r * propIr - Ir * (gamma_ir)
    ...
    dAr <- -muo * Ar + ((1-prob_lam)*lam * S) * (1 - prob_r) - tau_ar * Ar - gamma_ar * Ar
    ...
    inc = (prob_lam*lam * S) + ((1-prob_lam)*lam * S),
    inc_s = (prob_lam*lam * S) ,
    inc_r = ((1-prob_lam)*lam * S) ,
    inc_sym = ((prob_lam*lam * S) * (prob_s)) + (((1-prob_lam)*lam * S) * (prob_s)),
    inc_sym_s = ((prob_lam*lam * S) * (prob_s)),
    inc_sym_r =(((1-prob_lam)*lam * S) * (prob_s)),
    inc_asym = ((prob_lam*lam * S) * (1 - prob_s)) + (((1-prob_lam)*lam * S) * (1 - prob_r)),
    inc_asym_s =((prob_lam*lam * S) * (1 - prob_s)),
    inc_asym_r =(((1-prob_lam)*lam * S) * (1 - prob_r)),
